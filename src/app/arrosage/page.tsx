...
import { DEFAULT_AUTO_IRRIGATION_CONFIG, QUALITY_THRESHOLDS, formatPlanStatus, formatLocalTime, canExecutePlan, isPlanStale, estimateLiveHumidityPct } from "@/lib/irrigationScheduler";
import { canStartIrrigation, metersToCentimeters, type IrrigationGateResult } from "@/lib/irrigationSafety";
...
/** Bilan hydrique publié en continu par zone (reserveMm/capacityMm inclus) — jamais verrouillé, même executing/done. Voir checkAndPlanSchedule() dans le bridge. */
type LiveZoneState = ZoneMoistureState & { reserveMm: number; capacityMm: number; irrigationDepthMmPerHour: number };
type LiveWaterBalance = { updatedAt: string; zoneStates: LiveZoneState[] };

/** Confirmation manuelle en attente (persistée par le bridge) — sert à projeter l'humidité pendant un arrosage manuel en cours. */
...
const WATER_BALANCE_LIVE_PATH = "irrigation-ha/water-balance/live";
const WATER_BALANCE_PENDING_PATH = "irrigation-ha/water-balance/pending-manual";
...
function MoistureBar({ state, livePct }: { state?: ZoneMoistureState; livePct?: number }) {
  const isLive = livePct !== undefined;
  const pct = Math.max(0, Math.min(100, isLive ? livePct : state?.estimatedHumidityPct ?? 0));
  const decision = state?.decision;
  const color = isLive ? "bg-sky-500" : decision === "irrigate" ? "bg-amber-500" : decision === "wait" ? "bg-brand-600" : "bg-stone-300";
  return (
    <div>
      <div className="flex items-end justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-wide text-stone-400">
            {isLive ? "Estimation pendant l’arrosage" : "Humidité estimée"}
          </p>
          <p className="mt-1 text-2xl font-black tracking-tight text-stone-950">{state || isLive ? `${pct}%` : "—"}</p>
        </div>
        {isLive && (
          <span className="inline-flex items-center gap-1.5 rounded-full bg-sky-50 px-2.5 py-1 text-[10px] font-black text-sky-700 ring-1 ring-sky-100">
            <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-sky-500" /> en cours
          </span>
        )}
      </div>
      <div className="mt-2 h-2.5 overflow-hidden rounded-full bg-stone-100 ring-1 ring-stone-200">
        <div className={`h-full rounded-full ${color} ${isLive ? "transition-[width] duration-1000" : ""}`} style={{ width: `${pct}%` }} />
      </div>
    </div>
  );
}
...
function ZoneCompactCard({ zone, state, livePct }: { zone: ZoneConfig; state?: ZoneMoistureState; livePct?: number }) {
  const mode = zone.qualityMode ?? "passable";
  return (
    <article className="rounded-3xl border border-stone-200 bg-white p-4 ring-1 ring-white sm:p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={[
          ...
          livePct !== undefined ? "bg-sky-100 text-sky-800" : state?.decision === "irrigate" ? "bg-amber-100 text-amber-800" : state?.decision === "wait" ? "bg-brand-100 text-brand-800" : "bg-stone-200 text-stone-600",
        ].join(" ")}>{livePct !== undefined ? "Arrosage en cours" : state?.decision === "irrigate" ? "À arroser" : state?.decision === "wait" ? "Attendre" : "Estimée"}</span>
      </div>
      <div className="mt-4">
        <MoistureBar state={state} livePct={livePct} />
      </div>
    </article>
  );
}
...
function ManualSection({
  ...
  return (
    <div className="mt-5 space-y-5">
      ...
      {gateBlocked && (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-3 text-xs leading-relaxed text-red-800">
          <strong>Lancement manuel bloqué par la barrière de sécurité :</strong>
      ...
      <button
        disabled={refreshing || gateBlocked}
        onClick={onRun}
        className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-brand-700 px-4 text-sm font-black text-white shadow-lg shadow-brand-200 transition hover:bg-brand-800 active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-stone-300"
      >
        {refreshing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Power className="h-4 w-4" />}
        {gateBlocked ? "Lancement bloqué" : `Lancer zone ${zone}`}
      </button>
...
export default function ArrosagePage() {
  ...
  const [liveBalance, setLiveBalance] = useState<LiveWaterBalance | null>(null);
  const [pendingManual, setPendingManual] = useState<Record<string, PendingManualEntry>>({});
  const [gate, setGate] = useState<IrrigationGateResult | null>(null);
  ...
  // Clé de date stable : ne change (donc ne réabonne le listener plan) qu'au
  // franchissement réel du jour, pas à chaque tick de `now` (10s) — sinon le
  // listener Firebase du plan est recréé toutes les 10 secondes pour rien.
  ...
  // Instantané live du bilan hydrique (reserveMm/capacityMm par zone) — jamais
  // verrouillé, y compris pendant un plan "executing"/"done". Alimente
  // l'estimation en direct pendant un cycle (estimateLiveHumidityPct).
  useEffect(() => {
    const liveRef = ref(database, WATER_BALANCE_LIVE_PATH);
    return onValue(liveRef, (snap) => setLiveBalance(snap.val() as LiveWaterBalance | null));
  }, []);

  // Confirmations manuelles en attente — permet de retrouver le départ/la
  // durée/le débit d'un arrosage manuel en cours pour le projeter en direct.
  useEffect(() => {
    const pendingRef = ref(database, WATER_BALANCE_PENDING_PATH);
    return onValue(pendingRef, (snap) => setPendingManual((snap.val() as Record<string, PendingManualEntry> | null) ?? {}));
  }, []);

  // Barrière de sécurité recalculée côté client à partir des citernes en
  // direct (même logique pure canStartIrrigation que le bridge) — permet
  // d'afficher un bandeau motivé et de désactiver le lancement manuel sans
  // attendre un refus côté bridge. Le bridge revérifie de toute façon avant
  // tout appel HA réel : ceci n'est qu'un affichage anticipé, jamais la seule
  // barrière.
  useEffect(() => {
    let cancelled = false;
    async function checkGate() {
      try {
        const [c1, c2] = await Promise.all([loadCiterneStatus(1), loadCiterneStatus(2)]);
        if (cancelled) return;
        setGate(canStartIrrigation(
          { value: c1.niveau.value, freshness: c1.freshness },
          { value: metersToCentimeters(c2.hauteurEau.value), freshness: c2.freshness },
        ));
      } catch {
        // Impossible de vérifier les citernes depuis le navigateur : ne pas
        // bloquer l'UI sur une erreur réseau côté client, le bridge reste le
        // garde-fou réel côté serveur.
      }
    }
    void checkGate();
    const timer = window.setInterval(checkGate, 5 * 60 * 1000);
  ...
  async function action(body: Record<string, unknown>, success: string) {
    ...
    try {
      ...
      // Le refus réel (garde-fou citernes, entité non autorisée, erreur HA)
      // n'est connu qu'une fois le bridge traité la commande : on lit son
      // statut Firebase au lieu d'annoncer un succès optimiste.
      const commandId = commandRef.key;
      if (commandId) {
        const timeoutMs = 20000;
        let settled = false;
        let timeoutId: number | undefined;
        const unsubscribe = onValue(commandRef, (snap) => {
          const value = snap.val() as { status?: string; error?: string | null } | null;
          ...
          if (value.status === "done") {
            ...
            window.clearTimeout(timeoutId);
            setRefreshing(false);
            showToast({ type: "error", title: "Commande refusée par le bridge", message: value.error ?? "Erreur inconnue" });
          }
        });
  ...
  // Le bilan live (jamais verrouillé) prime sur plan.zoneStates (figé une fois
  // executing/done) ; fallback sur le plan tant que le bridge n'a pas encore
  // publié d'instantané live.
  const zoneStates = new Map((liveBalance?.zoneStates ?? todayPlan?.zoneStates ?? []).map((z) => [z.zone, z]));

  // Zone actuellement arrosée (auto verrouillé en "executing", ou manuel via
  // une confirmation en attente) — sert à projeter l'humidité en direct au
  // lieu d'afficher le dernier bilan confirmé (figé pendant tout le cycle).
  const activeIrrigation = useMemo(() => {
    const scheduledZones = (todayPlan?.zones ?? []) as Array<{ zone: 1 | 2; durationMinutes: number; status?: string; startedAt?: string }>;
    const executingZone = scheduledZones.find((z) => z.status === "executing" && z.startedAt);
    if (executingZone) {
      const zoneCfg = config.zones.find((z) => z.zone === executingZone.zone);
      return {
        zone: executingZone.zone,
        startedAtIso: executingZone.startedAt as string,
        plannedDurationMinutes: executingZone.durationMinutes,
        mmPerHour: zoneCfg?.irrigationDepthMmPerHour ?? 5,
      };
    }
    if (activeValve) {
      const manualEntry = Object.values(pendingManual).find((entry) => entry.zone === activeValve.zone);
      if (manualEntry) {
        return {
          zone: manualEntry.zone,
          startedAtIso: manualEntry.startedAt,
          plannedDurationMinutes: manualEntry.durationMinutes,
          mmPerHour: manualEntry.mmPerHour,
        };
      }
    }
    return null;
  }, [todayPlan, config.zones, activeValve, pendingManual]);

  const livePctByZone = useMemo(() => {
    const map = new Map<1 | 2, number>();
    if (activeIrrigation) {
      const liveState = liveBalance?.zoneStates.find((z) => z.zone === activeIrrigation.zone);
      if (liveState) {
        const projection = estimateLiveHumidityPct({
          confirmedState: { reserveMm: liveState.reserveMm, capacityMm: liveState.capacityMm },
          mmPerHour: activeIrrigation.mmPerHour,
          startedAtIso: activeIrrigation.startedAtIso,
          plannedDurationMinutes: activeIrrigation.plannedDurationMinutes,
          nowMs: now.getTime(),
        });
        map.set(activeIrrigation.zone, projection.pct);
      }
    }
    return map;
  }, [activeIrrigation, liveBalance, now]);

  const gateReasons = gate && !gate.allowed ? gate.reasons : [];
      ...
      <div className="mx-auto max-w-5xl space-y-5">
        ...
        {gateReasons.length > 0 && (
          <div className="rounded-3xl border border-red-200 bg-red-50 p-4 text-sm text-red-900">
            <div className="flex items-center gap-2 font-black"><AlertTriangle className="h-4 w-4" /> Arrosage bloqué par la barrière de sécurité</div>
        ...
        <section className="grid gap-3 sm:grid-cols-2">
          {config.zones.map((zone) => (
            <ZoneCompactCard key={zone.zone} zone={zone} state={zoneStates.get(zone.zone)} livePct={livePctByZone.get(zone.zone)} />
          ))}
        </section>
